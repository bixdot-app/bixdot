...
**Source (BXD-022 through BXD-028 below):** an independent, verified
technical/model audit received 2026-08-22, pinned to canonical commit
`772a7804770c0bb98ff99a62b945b02297406fcc` (main, after PR #27) and a separate
pass against a stale local checkout at `76ced3da8567584574cf7b2a0bbaa7bb73ce2b48`
(Phase 3 merge point). Both commit hashes verified real and in the exact
expected relationship. All eight technically-checkable claims below were
independently re-verified against current `main` before logging — see each
finding's evidence for the exact line numbers checked.
...
blocks the delete — a DB trigger in the current schema; confirmed present at
`core/audit/logger.py:151-160`, `no_delete`/`no_update`) and call
...
- Every schema-evolution `ALTER TABLE` statement (`:412-438`, six statements
  across three try/except blocks — the `email` column, the `persona_id`
  column, and a loop of four adding `password_scheme`, `password_changed_at`,
...
on a missing file rather than a code defect. (This friction is already
implicitly acknowledged in `CLAUDE.md`'s §19 backend-observability note,
which says local `cargo check` "needs a placeholder
`dist-backend/bixdot-backend-x86_64-pc-windows-msvc.exe`" — this finding
formalizes that as a tracked gap rather than a passing remark.)
